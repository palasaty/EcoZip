...
CBlowCbcCoder::CBlowCbcCoder(bool encodeMode, unsigned keySize):
  _keySize(keySize),
  _keyIsSet(false),
...
STDMETHODIMP CBlowCbcCoder::Init()
{
  return _keyIsSet ? S_OK : E_FAIL;
}

STDMETHODIMP_(UInt32) CBlowCbcCoder::Filter(Byte *data, UInt32 size)
{
  if (!_keyIsSet)
    return 0;
  if (size == 0)
  ...
  size >>= 4;
  if (_encodeMode)
      bl.Encrypt(data, data, size);
  else
      bl.Decrypt(data, data, size);
  return size << 4;
}

STDMETHODIMP CBlowCbcCoder::SetKey(const Byte *data, UInt32 size)
{
...
STDMETHODIMP CBlowCbcCoder::SetInitVector(const Byte *data, UInt32 size)
{
  return S_OK;
}

STDMETHODIMP CBlowCbcCoder::SetCoderProperties(const PROPID *propIDs, const PROPVARIANT *coderProps, UInt32 numProps)
{
